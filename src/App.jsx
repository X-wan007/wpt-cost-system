...
export default function App() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const canvas = await window.html2canvas(printRef.current, { scale: 2, useCORS: true, backgroundColor: '#ffffff' });
      const imgData = canvas.toDataURL('image/png');
      const pdf = new window.jspdf.jsPDF('p', 'mm', 'a4');
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
      pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
      pdf.save(`${activeTab === 'quotation' ? quoteSettings.quoteNumber : 'WPT-Report'}.pdf`);
    } catch (e) { showDialog('alert', 'เกิดข้อผิดพลาดในการสร้าง PDF'); }
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = `${activeTab === 'quotation' ? quoteSettings.quoteNumber : 'WPT-Report'}.jpg`;
      link.href = imgData;
      link.click();
    } catch (e) { showDialog('alert', 'เกิดข้อผิดพลาดในการสร้างรูปภาพ'); }
    setIsExporting(false);
  };
            ...
            <table className="w-full text-sm text-left text-gray-700 dark:text-gray-200">
              ...
              <tbody>
                {savedQuotations.length > 0 ? savedQuotations.map(q => (
                  <tr key={q.id} className="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-4 py-3 text-center">
                       <div className="flex justify-center gap-1">
                         <button onClick={() => handleEditQuote(q)} title="เปิด/แก้ไข" className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-700 rounded"><Edit size={16}/></button>
                         <button onClick={() => handleDuplicateQuote(q)} title="คัดลอกสร้างใหม่" className="p-1.5 text-yellow-600 hover:bg-yellow-50 dark:hover:bg-slate-700 rounded"><Copy size={16}/></button>
                         <button onClick={() => handleDeleteQuote(q.id)} title="ลบ" className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 rounded"><Trash2 size={16}/></button>
                       </div>
                    </td>
                  </tr>
  ...
  const renderQuotationPrint = () => {
    ...
    return (
      <div className="bg-white text-black text-sm p-8 max-w-[210mm] mx-auto min-h-[297mm] shadow-lg relative print:shadow-none print:m-0 print:w-full print:max-w-none print:p-0">
        ...
        {isPreviewModalOpen && (
          <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-sm flex flex-col items-center justify-center p-2 sm:p-4 print:bg-white print:p-0 print:block">
            ...
            <div className="bg-gray-200 dark:bg-slate-900 w-full flex-1 overflow-y-auto p-4 sm:p-8 flex justify-center print:bg-white print:p-0 print:overflow-visible print:block">
               <div ref={printRef} className={`bg-white w-full ${activeTab === 'quotation' ? 'max-w-[210mm] p-0' : 'max-w-[800px] p-6 sm:p-10'} shadow-lg rounded-sm text-slate-900 print:shadow-none print:max-w-none print:w-full`}>
