...
import { 
  ...
  Building2, User, ArrowUp, ArrowDown, Copy, ChevronLeft, ImageDown, Filter, Check, Archive, FileSpreadsheet
} from 'lucide-react';
...
// ย่อและบีบอัดรูปก่อนเก็บลง localStorage เพื่อลดโอกาส Safari ค้าง/จอดำ
const compressImageFile = async (file, options = {}) => {
  ...
  if (file.size > 20 * 1024 * 1024) throw new Error('รูปมีขนาดใหญ่เกิน 20 MB กรุณาเลือกรูปที่เล็กลง');
  ...
  try {
    ...
    const render = (w, h, quality) => {
      ...
      if (!ctx) throw new Error('อุปกรณ์ไม่รองรับการประมวลผลรูปภาพ');
    ...
    while (dataUrlBytes(dataUrl) > maxBytes && width > 640 && height > 640) {
      width = Math.round(width * 0.86);
      height = Math.round(height * 0.86);
      dataUrl = render(width, height, Math.max(minQuality, quality));
    }
...
const useLocalState = (key, defaultValue) => {
  const [value, setValue] = useState(() => {
    try {
      ...
      console.error(`อ่านข้อมูล ${key} ไม่สำเร็จ`, e);
      return defaultValue;
    }
  });
  useEffect(() => {
    try {
      ...
      console.error(`บันทึกข้อมูล ${key} ไม่สำเร็จ`, e);
      window.dispatchEvent(new CustomEvent('wpt-storage-error', {
        detail: { key, message: 'พื้นที่จัดเก็บในเครื่องใกล้เต็มหรือเต็ม กรุณาลบรูป/ข้อมูลเก่าบางส่วน แล้วลองใหม่' }
      }));
    }
  }, [key, value]);
...
const BANGKOK_METRO_PROVINCES = new Set([
  'กรุงเทพมหานคร','นนทบุรี','ปทุมธานี','สมุทรปราการ','สมุทรสาคร','นครปฐม'
]);

const WORK_TYPES = [
  { value: 'retaining', label: 'งานกำแพงกันดิน' },
  { value: 'fenceOnRetaining', label: 'งานรั้วบนกำแพงกันดิน' },
  { value: 'precastFence', label: 'งานรั้วคอนกรีตสำเร็จรูป' },
  { value: 'mixedFence', label: 'งานรั้วผสม' }
];

const MIXED_TYPES = ['คาวบอย','ระแนง','เหล็กกล่อง','ตะแกรง','ว่าง'];

const createProductSide = (name = 'A') => ({
  id: `${Date.now()}-${Math.random().toString(36).slice(2)}`,
  name,
  enabled: true,
  workType: 'precastFence',
  length: 30,
  span: 3,
  retainingHeight: 1,
  fenceHeight: 2,
...
export default function App() {
  ...
  const [productCalcProject, setProductCalcProject] = useLocalState('wpt_product_calc_project', {
    projectName: '', province: 'อุดรธานี'
  });
  const [productCalcSides, setProductCalcSides] = useLocalState('wpt_product_calc_sides', [
    createProductSide('A'), createProductSide('B'), createProductSide('C'), createProductSide('D')
  ]);
  const [productCalcActiveSide, setProductCalcActiveSide] = useState(0);
  const [productCalcHistory, setProductCalcHistory] = useLocalState('wpt_product_calc_history', []);
  ...
  const [companies, setCompanies] = useLocalState('wpt_companies', [
    { id: 'comp_default', logo: 'https://ui-avatars.com/api/?name=WPT&background=2563eb&color=fff&rounded=true', name: 'WPT ระบบรั้วสำเร็จรูป', address: '123 ถ.สุขุมวิท กรุงเทพฯ 10110', taxId: '0105555555555', phone: '02-123-4567', bankName: 'ธนาคารกสิกรไทย', accountNo: '123-4-56789-0', accountName: 'บจก. WPT รั้วสำเร็จรูป', preparerName: '', preparerSignature: '' }
  ]);
  ...
  const [quoteApprovalFilter, setQuoteApprovalFilter] = useState('all');
  const [backupRange, setBackupRange] = useState('1');
  ...
  // ข้อความสำเร็จรูปสำหรับหมายเหตุและเงื่อนไขการชำระเงิน
  const [remarkTemplates, setRemarkTemplates] = useLocalState('wpt_remark_templates', [
    'ยืนราคา 30 วันนับจากวันที่เสนอราคา',
    'ราคานี้ไม่รวมงานเพิ่มเติมนอกเหนือจากรายการในใบเสนอราคา'
  ]);
  const [paymentTemplates, setPaymentTemplates] = useLocalState('wpt_payment_templates', [
    'มัดจำ 50% ก่อนเริ่มงาน, 50% เมื่องานแล้วเสร็จ',
    'ชำระมัดจำ 30% ก่อนเริ่มงาน, 40% ระหว่างดำเนินงาน และ 30% เมื่องานแล้วเสร็จ'
  ]);
  const [selectedRemarkTemplate, setSelectedRemarkTemplate] = useState('');
  ...
  useEffect(() => {
    const handleStorageError = (event) => {
      showDialog('alert', event.detail?.message || 'ไม่สามารถบันทึกข้อมูลลงเครื่องได้ กรุณาตรวจสอบพื้นที่จัดเก็บ');
    };
    window.addEventListener('wpt-storage-error', handleStorageError);
    return () => window.removeEventListener('wpt-storage-error', handleStorageError);
  }, []);
  ...
  const handleCreateNewQuote = () => {
    ...
    setQuoteSettings({
      ...
      preparerName: company?.preparerName || '',
      preparerSignature: company?.preparerSignature || ''
    });
  ...
  const handleSaveQuotationDoc = () => {
    ...
    setTimeout(() => {
      ...
      const newDoc = {
        ...
        total: grandTotal,
        approved: editingQuoteId ? (savedQuotations.find(q => q.id === editingQuoteId)?.approved ?? previewApproved) : previewApproved,
        timestamp: Date.now()
      };
  ...
  const openDbForm = (item = null) => {
    ...
    setProductImageInfo(item?.image ? 'รูปภาพเดิมถูกบันทึกไว้แล้ว' : '');
    setIsQuoteFormModalOpen(true);
  };
  const handleDbImageUpload = async (e) => {
    const input = e.target;
    const file = input.files?.[0];
    if (!file) return;

    setIsCompressingProductImage(true);
    setProductImageInfo('กำลังย่อและบีบอัดรูป...');
    try {
      ...
      const compressedKB = Math.round(result.bytes / 1024);
      setDbFormData(prev => ({ ...prev, image: result.dataUrl }));
      setProductImageInfo(`บีบอัดแล้ว ${originalKB.toLocaleString()} KB → ${compressedKB.toLocaleString()} KB (${result.width}×${result.height}px)`);
    } catch (error) {
      console.error('Product image compression failed', error);
      setProductImageInfo('');
      showDialog('alert', error?.message || 'ไม่สามารถอัปโหลดรูปภาพได้ กรุณาลองเลือกรูปใหม่');
    } finally {
  ...
  // --- Company / Bank synchronization ---
  const applyCompanyBankToQuote = (company) => {
    if (!company) return;
  ...
  useEffect(() => {
    ...
    if (selectedCompany) applyCompanyBankToQuote(selectedCompany);
  }, [activeCompanyId]);

  const saveTextTemplate = (type) => {
    const text = (type === 'remark' ? quoteSettings.remark : quoteSettings.paymentTerms).trim();
    if (!text) return showDialog('alert', 'กรุณากรอกข้อความก่อนบันทึก');
    if (type === 'remark') {
      if (!remarkTemplates.includes(text)) setRemarkTemplates([...remarkTemplates, text]);
      setSelectedRemarkTemplate(text);
    } else {
      if (!paymentTemplates.includes(text)) setPaymentTemplates([...paymentTemplates, text]);
      setSelectedPaymentTemplate(text);
    }
  };

  const deleteTextTemplate = (type) => {
    const selected = type === 'remark' ? selectedRemarkTemplate : selectedPaymentTemplate;
    if (!selected) return showDialog('alert', 'กรุณาเลือกรายการที่ต้องการลบ');
    if (type === 'remark') {
      setRemarkTemplates(remarkTemplates.filter(item => item !== selected));
      setSelectedRemarkTemplate('');
    } else {
      setPaymentTemplates(paymentTemplates.filter(item => item !== selected));
  ...
  const handleSaveCompany = () => {
    ...
    const normalizedCompany = {
      ...
      accountNo: editingCompany.accountNo || '',
      accountName: editingCompany.accountName || '',
      preparerName: editingCompany.preparerName || '',
      preparerSignature: editingCompany.preparerSignature || ''
    };
    if (editingCompany.id) {
      setCompanies(prev => prev.map(c => c.id === editingCompany.id ? normalizedCompany : c));
      if (editingCompany.id === activeCompanyId) applyCompanyBankToQuote(normalizedCompany);
    } else {
      const newComp = { ...normalizedCompany, id: `comp_${Date.now()}` };
      setCompanies([...companies, newComp]);
      setActiveCompanyId(newComp.id);
      applyCompanyBankToQuote(newComp);
    }
  ...
  const handleCompanySignatureUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => setEditingCompany({...editingCompany, preparerSignature: reader.result});
      reader.readAsDataURL(file);
    }
  };
  ...
  const toggleQuoteApproval = (id) => {
    setSavedQuotations(prev => prev.map(q => q.id === id ? {...q, approved: !q.approved} : q));
  };

  const getBackupQuotes = () => {
    const months = parseInt(backupRange, 10) || 1;
    const from = new Date();
    from.setMonth(from.getMonth() - months);
    return savedQuotations.filter(q => new Date(q.settings?.date || q.timestamp) >= from);
  };

  const finalizeBackup = (quotes) => {
    if (!deleteAfterBackup || quotes.length === 0) return;
    showDialog('confirm', `ส่งออกสำเร็จแล้ว ต้องการลบข้อมูลที่แบ็คอัพจำนวน ${quotes.length} รายการออกจากระบบหรือไม่?`, () => {
      const ids = new Set(quotes.map(q => q.id));
      setSavedQuotations(prev => prev.filter(q => !ids.has(q.id)));
    });
  };

  const handleBackupExcel = async () => {
    const quotes = getBackupQuotes();
    if (!quotes.length) return showDialog('alert', 'ไม่พบใบเสนอราคาในช่วงเวลาที่เลือก');
    try {
      await loadScript('https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js');
      const rows = quotes.map((q, index) => ({
        'ลำดับ': index + 1,
        'เลขที่เอกสาร': q.settings?.quoteNumber || '',
        'วันที่': q.settings?.date || '',
        'ชื่อลูกค้า': q.customer?.contactName || '',
        'บริษัทลูกค้า': q.customer?.companyName || '',
        'โครงการ': q.customer?.project || '',
        'ผู้เสนอราคา': q.settings?.preparerName || '',
        'ยอดสุทธิ': q.total || 0,
        'สถานะ': q.approved ? 'อนุมัติ' : 'รออนุมัติ'
      }));
      const ws = window.XLSX.utils.json_to_sheet(rows);
      const wb = window.XLSX.utils.book_new();
      window.XLSX.utils.book_append_sheet(wb, ws, 'ใบเสนอราคา');
      window.XLSX.writeFile(wb, `WPT-quotation-backup-${new Date().toISOString().slice(0,10)}.xlsx`);
      finalizeBackup(quotes);
    } catch (e) { console.error(e); showDialog('alert', 'ไม่สามารถสร้างไฟล์ Excel ได้'); }
  };
  ...
  const handleBackupPDF = async () => {
    ...
    try {
      ...
      let y = 16;
      pdf.setFontSize(16); pdf.text('WPT Quotation Backup', 14, y); y += 10;
      pdf.setFontSize(9);
      quotes.forEach((q, i) => {
        if (y > 278) { pdf.addPage(); y = 16; }
        const line = `${i+1}. ${q.settings?.quoteNumber || '-'} | ${q.settings?.date || '-'} | ${q.customer?.contactName || q.customer?.companyName || '-'} | ${Number(q.total||0).toLocaleString()} THB | ${q.approved ? 'APPROVED' : 'PENDING'}`;
        const parts = pdf.splitTextToSize(line, 182);
        pdf.text(parts, 14, y); y += parts.length * 5 + 2;
      });
      pdf.save(`WPT-quotation-backup-${new Date().toISOString().slice(0,10)}.pdf`);
      finalizeBackup(quotes);
    } catch (e) { console.error(e); showDialog('alert', 'ไม่สามารถสร้างไฟล์ PDF ได้'); }
  };

  const filteredDbItems = quoteDatabase.filter(item => dbTypeFilter === 'all' || item.type === dbTypeFilter);
  const filteredSelectorItems = quoteDatabase.filter(item => selectorTypeFilter === 'all' || item.type === selectorTypeFilter);

  // --- Export / Print ---
  const getA4PageCanvases = async () => {
    await loadScript('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js');

    if (!printRef.current) throw new Error('ไม่พบพื้นที่เอกสารสำหรับบันทึก');

    // รอให้ฟอนต์และรูปภาพในใบเสนอราคาโหลดครบก่อนจับภาพ
    ...
    const sourceCanvas = await window.html2canvas(printRef.current, {
      ...
      windowWidth: Math.max(printRef.current.scrollWidth, 1200),
      windowHeight: printRef.current.scrollHeight,
    });

    // ขนาด A4 แนวตั้งที่ความละเอียดประมาณ 192 DPI (html2canvas scale 2)
    ...
    // กรณีเนื้อหาเกิน A4 เพียงเล็กน้อย เช่น ลายเซ็นตกหน้า 2:
    // ย่อทั้งเอกสารให้พอดี A4 หน้าเดียว แทนการสร้างหน้าที่ 2 เกือบว่าง
    const widthScale = pageWidth / sourceCanvas.width;
    const scaledFullHeight = sourceCanvas.height * widthScale;
    const singlePageTolerance = pageHeight * 1.18;

    if (scaledFullHeight <= singlePageTolerance) {
      const { canvas, ctx } = createBlankPage();
      const fitScale = Math.min(pageWidth / sourceCanvas.width, pageHeight / sourceCanvas.height);
      const drawWidth = Math.floor(sourceCanvas.width * fitScale);
      const drawHeight = Math.floor(sourceCanvas.height * fitScale);
      const offsetX = Math.floor((pageWidth - drawWidth) / 2);
      ctx.drawImage(sourceCanvas, 0, 0, sourceCanvas.width, sourceCanvas.height, offsetX, 0, drawWidth, drawHeight);
      pages.push(canvas);
      return pages;
    }

    // กรณีรายการยาวจริง ให้ต่อหน้า 2, 3 ... โดยรักษาความกว้างให้พอดี A4
    const scale = pageWidth / sourceCanvas.width;
    const sourceSliceHeight = Math.floor(pageHeight / scale);

    for (let offsetY = 0; offsetY < sourceCanvas.height; offsetY += sourceSliceHeight) {
      const sliceHeight = Math.min(sourceSliceHeight, sourceCanvas.height - offsetY);
      const { canvas, ctx } = createBlankPage();
      const drawHeight = Math.floor(sliceHeight * scale);
      ctx.drawImage(
        sourceCanvas,
        0, offsetY, sourceCanvas.width, sliceHeight,
        0, 0, pageWidth, drawHeight
      );
      pages.push(canvas);
    }
  ...
  const roundUpHalf = (value) => Math.ceil((Number(value) || 0) * 2) / 2;
  const numeric = (value, fallback = 0) => Number.isFinite(Number(value)) ? Number(value) : fallback;

  const calculateProductSide = (side) => {
    const length = Math.max(0, numeric(side.length));
    const requestedSpan = Math.max(0.1, numeric(side.span, 3));
    const bays = length > 0 ? Math.ceil(length / requestedSpan) : 0;
    const posts = bays > 0 ? bays + 1 : 0;
    const actualSpan = bays > 0 ? length / bays : 0;
    const retainingHeight = Math.max(0, numeric(side.retainingHeight));
    const fenceHeight = Math.max(0, numeric(side.fenceHeight));
    const isMetro = BANGKOK_METRO_PROVINCES.has(productCalcProject.province);
    const pileLength = isMetro ? 6 : Math.min(6, Math.max(3, roundUpHalf(retainingHeight + 2)));
    const retainingPanelsPerBay = Math.max(0, Math.ceil(Math.max(0, retainingHeight - 0.30) / 0.35 - 1e-9));
    const fencePostLength = Math.ceil((fenceHeight + 0.20) * 10) / 10;
    const rawFencePanels = Math.max(0, Math.ceil(fenceHeight / 0.25 - 1e-9));
    const fencePanelsPerBay = Math.max(0, rawFencePanels - (side.topType === 'tee' ? 1 : 0));
    const stayQty = length > 0 && numeric(side.staySpacing) > 0 ? Math.ceil(length / numeric(side.staySpacing)) : 0;
    const result = [];
    const add = (key, label, qty, unit, spec = '') => result.push({ key, label, qty: Number(qty) || 0, unit, spec });

    add('bays', 'จำนวนช่อง', bays, 'ช่อง', `ระยะเฉลี่ย ${actualSpan.toFixed(2)} ม.`);

    if (side.workType === 'retaining' || side.workType === 'fenceOnRetaining') {
      add('mainPiles', `เสาเข็มไอ ${side.pileSize}x${side.pileSize} ซม.`, posts, 'ต้น', `ยาว ${pileLength.toFixed(2)} ม.`);
      add('retainingPanels', 'แผ่นกำแพงกันดิน สูง 35 ซม.', bays * retainingPanelsPerBay, 'แผ่น', `${retainingPanelsPerBay} แผ่น/ช่อง`);
      add('groundBeams', 'คานคอนกรีต สูง 30 ซม.', bays, 'ท่อน', `ยาวเฉลี่ย ${actualSpan.toFixed(2)} ม.`);
      add('stayPiles', `เข็มสเตย์ไอ ${side.pileSize}x${side.pileSize} ซม.`, stayQty, 'ต้น', `ยาว ${pileLength.toFixed(2)} ม. ทุก ${numeric(side.staySpacing).toFixed(2)} ม.`);
    }

    if (side.workType === 'precastFence' || side.workType === 'fenceOnRetaining' || side.workType === 'mixedFence') {
      add('fencePosts', 'เสารั้วไอ 15x15 ซม.', posts, 'ต้น', `ยาว ${fencePostLength.toFixed(2)} ม.`);
      if (side.workType === 'mixedFence') {
        add('mixedInfill', `ช่องรั้วผสม — ${side.mixedType}`, bays, 'ช่อง', `ความสูง ${fenceHeight.toFixed(2)} ม.`);
      } else {
        add('fencePanels', 'แผ่นรั้วคอนกรีต สูง 25 ซม.', bays * fencePanelsPerBay, 'แผ่น', `${fencePanelsPerBay} แผ่น/ช่อง`);
        add('topBeam', side.topType === 'tee' ? 'ทับหลังตัวที สูง 29 ซม.' : 'ทับหลังครอบ', bays, 'ท่อน', side.topType === 'tee' ? 'แทนแผ่นรั้ว 1 แผ่น/ช่อง' : 'ไม่หักจำนวนแผ่นรั้ว');
      }
      if (fenceHeight >= 2.50) add('db12', 'เหล็กเสริม DB12 ยาว 1.00 ม.', posts * 4, 'เส้น', '4 เส้น/เสา');
    }
    return result;
  };

  const recalculateProductSide = (index) => {
    setProductCalcSides(prev => prev.map((side, i) => i === index ? { ...side, results: calculateProductSide(side) } : side));
  };

  const recalculateAllProductSides = () => {
    setProductCalcSides(prev => prev.map(side => ({ ...side, results: side.enabled ? calculateProductSide(side) : [] })));
  };

  const updateProductSide = (index, field, value) => {
    setProductCalcSides(prev => prev.map((side, i) => i === index ? { ...side, [field]: value } : side));
  };

  const updateProductResult = (sideIndex, rowIndex, value) => {
    setProductCalcSides(prev => prev.map((side, i) => {
      if (i !== sideIndex) return side;
      const baseResults = side.results.length ? side.results : calculateProductSide(side);
      return {
        ...side,
        results: baseResults.map((row, ri) => ri === rowIndex ? { ...row, qty: Math.max(0, numeric(value)) } : row)
      };
    }));
  };

  const addOtherProductSide = () => {
    setProductCalcSides(prev => [...prev, createProductSide(`อื่นๆ ${prev.filter(s => s.name.startsWith('อื่นๆ')).length + 1}`)]);
    setProductCalcActiveSide(productCalcSides.length);
  };

  const removeProductSide = (index) => {
    if (index < 4) return;
    setProductCalcSides(prev => prev.filter((_, i) => i !== index));
    setProductCalcActiveSide(0);
  };

  const productSummaryRows = useMemo(() => {
    const map = new Map();
    productCalcSides.filter(s => s.enabled).forEach(side => {
      const rows = side.results.length ? side.results : calculateProductSide(side);
      rows.filter(r => r.key !== 'bays').forEach(row => {
        const id = `${row.key}|${row.label}|${row.spec}|${row.unit}`;
        if (!map.has(id)) map.set(id, { ...row, qty: 0, sides: [] });
        const target = map.get(id);
        target.qty += numeric(row.qty);
        target.sides.push(side.name);
      });
    });
    return Array.from(map.values());
  }, [productCalcSides, productCalcProject.province]);

  const saveProductCalculation = () => {
    if (!productCalcProject.projectName.trim()) return showDialog('alert', 'กรุณากรอกชื่อโครงการก่อนบันทึก');
    const calculatedSides = productCalcSides.map(side => ({
      ...side,
      results: side.enabled ? (side.results.length ? side.results : calculateProductSide(side)) : []
    }));
    const summaryMap = new Map();
    calculatedSides.filter(side => side.enabled).forEach(side => {
      side.results.filter(row => row.key !== 'bays').forEach(row => {
        const id = `${row.key}|${row.label}|${row.spec}|${row.unit}`;
        if (!summaryMap.has(id)) summaryMap.set(id, { ...row, qty: 0, sides: [] });
        const target = summaryMap.get(id);
        target.qty += numeric(row.qty);
        target.sides.push(side.name);
      });
    });
    setProductCalcSides(calculatedSides);
    const entry = {
      id: Date.now(),
      project: { ...productCalcProject },
      sides: calculatedSides,
      summary: Array.from(summaryMap.values()),
      date: new Date().toLocaleString('th-TH')
    };
    setProductCalcHistory(prev => [entry, ...prev].slice(0, 50));
    showDialog('alert', 'บันทึกการคำนวณสินค้าเรียบร้อยแล้ว');
  };

  const loadProductCalculation = (entry) => {
    setProductCalcProject(entry.project);
    setProductCalcSides(entry.sides);
    setProductCalcActiveSide(0);
    setActiveTab('productCalc');
  };
  ...
  const TABS = [
    ...
    { id: 'productCalc', label: 'คำนวณสินค้า', icon: Calculator, isNew: true },
  ...
  const renderProductCalculator = () => {
    const side = productCalcSides[productCalcActiveSide] || productCalcSides[0];
    if (!side) return null;
    const workLabel = WORK_TYPES.find(w => w.value === side.workType)?.label || '';
    const sideResults = side.results.length ? side.results : calculateProductSide(side);
    const inputClass = 'w-full rounded-lg border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2.5 text-sm text-slate-800 dark:text-slate-100 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15';
    const labelClass = 'block text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1.5';

    return (
      <div className="space-y-5 animate-in fade-in duration-300">
        <section className="rounded-2xl border border-gray-200 dark:border-blue-400/15 bg-white dark:bg-slate-900/70 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-200 dark:border-slate-700 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2"><Calculator className="text-amber-500" size={23}/><h2 className="text-xl font-bold text-slate-900 dark:text-white">คำนวณสินค้า</h2><span className="text-[10px] font-black px-2 py-1 rounded-full bg-amber-400 text-slate-950">NEW</span></div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">คำนวณจำนวนช่อง เสา แผ่น คาน ทับหลัง สเตย์ และเหล็กเสริม แยกแต่ละด้าน</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <button onClick={recalculateAllProductSides} className="px-4 py-2.5 rounded-lg border border-blue-500/30 text-blue-600 dark:text-cyan-300 bg-blue-50 dark:bg-blue-500/10 font-semibold text-sm hover:bg-blue-100 dark:hover:bg-blue-500/20">คำนวณใหม่ทุกด้าน</button>
              <button onClick={saveProductCalculation} className="px-4 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm flex items-center gap-2"><Save size={16}/>บันทึกงาน</button>
            </div>
          </div>
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={labelClass}>ชื่อโครงการ</label><input value={productCalcProject.projectName} onChange={e=>setProductCalcProject({...productCalcProject, projectName:e.target.value})} placeholder="ระบุชื่อโครงการ..." className={inputClass}/></div>
            <div><label className={labelClass}>จังหวัด</label><select value={productCalcProject.province} onChange={e=>setProductCalcProject({...productCalcProject, province:e.target.value})} className={inputClass}>{THAI_PROVINCES.map(p=><option key={p}>{p}</option>)}</select><p className="mt-1 text-[11px] text-gray-400">{BANGKOK_METRO_PROVINCES.has(productCalcProject.province) ? 'กรุงเทพฯ/ปริมณฑล: ระบบกำหนดเข็มหลัก 6.00 ม.' : 'ต่างจังหวัด: ความยาวเข็ม = ฝัง 2.00 ม. + ความสูงกันดิน (ขั้นต่ำ 3.00 ม.)'}</p></div>
          </div>
        </section>

        <div className="flex gap-2 overflow-x-auto pb-1">
          {productCalcSides.map((s,i)=><button key={s.id} onClick={()=>setProductCalcActiveSide(i)} className={`shrink-0 px-4 py-2.5 rounded-xl border font-bold text-sm flex items-center gap-2 ${i===productCalcActiveSide?'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-600/20':'bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300'}`}><span className={`w-2 h-2 rounded-full ${s.enabled?'bg-emerald-400':'bg-slate-500'}`}></span>ด้าน {s.name}</button>)}
          <button onClick={addOtherProductSide} className="shrink-0 px-4 py-2.5 rounded-xl border border-dashed border-blue-400/50 text-blue-600 dark:text-blue-300 font-semibold text-sm"><Plus size={15} className="inline mr-1"/>อื่นๆ</button>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_.9fr] gap-5">
          <section className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
              <div><h3 className="font-bold text-slate-900 dark:text-white">ข้อมูลด้าน {side.name}</h3><p className="text-xs text-gray-500 dark:text-slate-400">{workLabel}</p></div>
              <div className="flex items-center gap-3"><label className="text-xs text-gray-500 dark:text-slate-400 flex items-center gap-2"><input type="checkbox" checked={side.enabled} onChange={e=>updateProductSide(productCalcActiveSide,'enabled',e.target.checked)}/>ใช้งานด้านนี้</label>{productCalcActiveSide>=4&&<button onClick={()=>removeProductSide(productCalcActiveSide)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg"><Trash2 size={17}/></button>}</div>
            </div>
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><label className={labelClass}>ชื่อด้าน</label><input value={side.name} onChange={e=>updateProductSide(productCalcActiveSide,'name',e.target.value)} className={inputClass}/></div>
              <div><label className={labelClass}>ประเภทงาน</label><select value={side.workType} onChange={e=>updateProductSide(productCalcActiveSide,'workType',e.target.value)} className={inputClass}>{WORK_TYPES.map(w=><option key={w.value} value={w.value}>{w.label}</option>)}</select></div>
              <div><label className={labelClass}>ความยาวรั้ว (เมตร)</label><input type="number" step="0.01" value={side.length} onChange={e=>updateProductSide(productCalcActiveSide,'length',e.target.value)} className={inputClass}/></div>
              <div><label className={labelClass}>ระยะช่องที่ต้องการ (เมตร)</label><input type="number" step="0.01" value={side.span} onChange={e=>updateProductSide(productCalcActiveSide,'span',e.target.value)} className={inputClass}/></div>
              {(side.workType==='retaining'||side.workType==='fenceOnRetaining')&&<>
                <div><label className={labelClass}>ความสูงกำแพงกันดิน (เมตร)</label><input type="number" step="0.05" value={side.retainingHeight} onChange={e=>updateProductSide(productCalcActiveSide,'retainingHeight',e.target.value)} className={inputClass}/></div>
                <div><label className={labelClass}>ขนาดเสาเข็ม</label><select value={side.pileSize} onChange={e=>updateProductSide(productCalcActiveSide,'pileSize',e.target.value)} className={inputClass}><option value="18">ไอ 18x18 ซม.</option><option value="22">ไอ 22x22 ซม.</option></select></div>
                <div><label className={labelClass}>ระยะเข็มสเตย์ (เมตร)</label><input type="number" step="0.5" value={side.staySpacing} onChange={e=>updateProductSide(productCalcActiveSide,'staySpacing',e.target.value)} className={inputClass}/></div>
              </>}
              {(side.workType==='precastFence'||side.workType==='fenceOnRetaining'||side.workType==='mixedFence')&&<>
                <div><label className={labelClass}>ความสูงรั้ว (เมตร)</label><input type="number" step="0.05" value={side.fenceHeight} onChange={e=>updateProductSide(productCalcActiveSide,'fenceHeight',e.target.value)} className={inputClass}/></div>
                {side.workType!=='mixedFence'&&<div><label className={labelClass}>รูปแบบทับหลัง</label><select value={side.topType} onChange={e=>updateProductSide(productCalcActiveSide,'topType',e.target.value)} className={inputClass}><option value="cap">ทับหลังครอบ</option><option value="tee">ทับหลังตัวที</option></select></div>}
                {side.workType==='mixedFence'&&<div><label className={labelClass}>ชนิดรั้วผสม</label><select value={side.mixedType} onChange={e=>updateProductSide(productCalcActiveSide,'mixedType',e.target.value)} className={inputClass}>{MIXED_TYPES.map(x=><option key={x}>{x}</option>)}</select></div>}
              </>}
            </div>
            <div className="px-5 pb-5"><button disabled={!side.enabled} onClick={()=>recalculateProductSide(productCalcActiveSide)} className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold disabled:opacity-40"><Calculator size={18} className="inline mr-2"/>คำนวณด้าน {side.name}</button></div>
          </section>

          <section className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700"><h3 className="font-bold text-slate-900 dark:text-white">ผลคำนวณด้าน {side.name}</h3><p className="text-xs text-gray-500 dark:text-slate-400">ตัวเลขทุกช่องสามารถแก้ไขเองได้</p></div>
            <div className="divide-y divide-gray-100 dark:divide-slate-800 max-h-[620px] overflow-auto">
              {sideResults.map((row,ri)=><div key={`${row.key}-${ri}`} className="p-4 grid grid-cols-[1fr_105px] gap-3 items-center"><div><div className="text-sm font-semibold text-slate-800 dark:text-slate-100">{row.label}</div><div className="text-[11px] text-gray-500 dark:text-slate-400 mt-0.5">{row.spec}</div></div><div className="flex items-center gap-2"><input type="number" step="1" value={row.qty} onChange={e=>updateProductResult(productCalcActiveSide,ri,e.target.value)} className="w-20 rounded-lg border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-950 px-2 py-2 text-right font-bold text-blue-600 dark:text-cyan-300 outline-none"/><span className="text-xs text-gray-500 w-10">{row.unit}</span></div></div>)}
            </div>
          </section>
        </div>

        <section className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700"><h3 className="font-bold text-slate-900 dark:text-white">ภาพจำลององค์ประกอบด้าน {side.name}</h3><p className="text-xs text-gray-500 dark:text-slate-400">ภาพประกอบเชิงระบบ ไม่ใช่แบบก่อสร้างจริง</p></div>
          <div className="p-5">
            <div className="rounded-xl border border-blue-400/20 bg-gradient-to-b from-sky-100 to-slate-100 dark:from-slate-950 dark:to-slate-900 min-h-[280px] p-6 flex flex-col justify-end overflow-hidden relative">
              <div className="absolute top-4 left-4 text-xs font-bold text-blue-700 dark:text-cyan-300">ด้าน {side.name} · {numeric(side.length).toFixed(2)} ม. · {workLabel}</div>
              {(side.workType==='precastFence'||side.workType==='fenceOnRetaining')&&<div className="mx-auto w-[88%] relative">
                <div className={`h-5 rounded-t-md border-2 border-slate-500 dark:border-slate-400 ${side.topType==='tee'?'bg-amber-300':'bg-slate-300 dark:bg-slate-500'}`}></div>
                {Array.from({length:Math.min(8,Math.max(1,Math.ceil(numeric(side.fenceHeight)/.25)-(side.topType==='tee'?1:0)))}).map((_,i)=><div key={i} className="h-6 bg-slate-200 dark:bg-slate-600 border-x-2 border-b border-slate-400 dark:border-slate-500"></div>)}
                <div className="absolute -left-3 -top-2 w-6 h-[calc(100%+16px)] bg-slate-400 dark:bg-slate-500 border-2 border-slate-600 rounded-sm"></div><div className="absolute -right-3 -top-2 w-6 h-[calc(100%+16px)] bg-slate-400 dark:bg-slate-500 border-2 border-slate-600 rounded-sm"></div>
              </div>}
              {side.workType==='mixedFence'&&<div className="mx-auto w-[88%] h-28 border-4 border-slate-600 relative bg-white/30 dark:bg-slate-800/30 flex items-center justify-center"><span className="font-bold text-slate-600 dark:text-slate-300">รั้วผสม: {side.mixedType}</span><div className="absolute -left-4 -top-4 w-7 h-36 bg-slate-500"></div><div className="absolute -right-4 -top-4 w-7 h-36 bg-slate-500"></div></div>}
              {(side.workType==='retaining'||side.workType==='fenceOnRetaining')&&<div className="mx-auto w-[88%] relative">
                {Array.from({length:Math.min(5,Math.max(1,Math.ceil(Math.max(0,numeric(side.retainingHeight)-.3)/.35)))}).map((_,i)=><div key={i} className="h-8 bg-stone-300 dark:bg-stone-600 border-2 border-stone-500"></div>)}
                <div className="h-8 bg-stone-500 dark:bg-stone-700 border-2 border-stone-700 flex items-center justify-center text-[10px] text-white">คาน 30 ซม.</div>
                <div className="absolute -left-3 top-0 w-6 h-[calc(100%+60px)] bg-stone-500 border-2 border-stone-700"></div><div className="absolute -right-3 top-0 w-6 h-[calc(100%+60px)] bg-stone-500 border-2 border-stone-700"></div>
              </div>}
              <div className="h-3 bg-emerald-700/70 mt-12 rounded-full"></div>
            </div>
          </div>
        </section>

        <section className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex justify-between"><div><h3 className="font-bold text-slate-900 dark:text-white">ยอดรวมสินค้าทุกด้าน</h3><p className="text-xs text-gray-500 dark:text-slate-400">รวมเฉพาะด้านที่เปิดใช้งาน</p></div><span className="text-xs font-bold px-3 py-1.5 rounded-full bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-cyan-300">{productCalcSides.filter(s=>s.enabled).length} ด้าน</span></div>
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-slate-950/50 text-gray-500 dark:text-slate-400 text-xs"><tr><th className="px-4 py-3 text-left">รายการ</th><th className="px-4 py-3 text-left">รายละเอียด</th><th className="px-4 py-3 text-right">จำนวนรวม</th><th className="px-4 py-3 text-center">หน่วย</th></tr></thead><tbody className="divide-y divide-gray-100 dark:divide-slate-800">{productSummaryRows.map((row,i)=><tr key={i}><td className="px-4 py-3 font-semibold text-slate-800 dark:text-slate-100">{row.label}</td><td className="px-4 py-3 text-xs text-gray-500 dark:text-slate-400">{row.spec}</td><td className="px-4 py-3 text-right font-black text-blue-600 dark:text-cyan-300">{numeric(row.qty).toLocaleString()}</td><td className="px-4 py-3 text-center text-gray-500">{row.unit}</td></tr>)}</tbody></table></div>
        </section>
      </div>
    );
  };
  ...
  const renderProductHistory = () => (
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold text-slate-900 dark:text-white">ประวัติ</h2><p className="text-sm text-gray-500 dark:text-slate-400">งานคำนวณสินค้าที่บันทึกไว้ในอุปกรณ์นี้</p></div><span className="px-3 py-1.5 rounded-full bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-cyan-300 text-xs font-bold">{productCalcHistory.length} รายการ</span></div>
      {productCalcHistory.length===0?<div className="rounded-2xl border border-dashed border-gray-300 dark:border-slate-700 p-12 text-center text-gray-500 dark:text-slate-400"><Archive size={38} className="mx-auto mb-3 opacity-50"/><p>ยังไม่มีประวัติการคำนวณสินค้า</p></div>:productCalcHistory.map(entry=><div key={entry.id} className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 p-5 flex flex-col md:flex-row md:items-center gap-4"><div className="flex-1"><h3 className="font-bold text-slate-900 dark:text-white">{entry.project.projectName}</h3><p className="text-sm text-gray-500 dark:text-slate-400">{entry.project.province} · {entry.sides.filter(s=>s.enabled).length} ด้าน · {entry.date}</p></div><div className="text-sm text-gray-500 dark:text-slate-400">สินค้า {entry.summary?.length||0} รายการ</div><button onClick={()=>loadProductCalculation(entry)} className="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold text-sm flex items-center justify-center gap-2"><Copy size={15}/>เปิด/คัดลอกงาน</button><button onClick={()=>setProductCalcHistory(prev=>prev.filter(x=>x.id!==entry.id))} className="p-2.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10"><Trash2 size={17}/></button></div>)}
    </div>
  );
  ...
  const renderQuotationList = () => {
    const monthOptions = [...new Set(savedQuotations.map(q => (q.settings?.date || '').slice(0,7)).filter(Boolean))].sort().reverse();
    const filteredQuotes = savedQuotations.filter(q => {
      const monthOk = quoteMonthFilter === 'all' || (q.settings?.date || '').startsWith(quoteMonthFilter);
      const approvalOk = quoteApprovalFilter === 'all' || (quoteApprovalFilter === 'approved' ? !!q.approved : !q.approved);
      return monthOk && approvalOk;
    });
    return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-wrap justify-between items-center gap-3">
         ...
         <div className="flex flex-wrap gap-2">
           <button onClick={handleBackupExcel} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-sm"><FileSpreadsheet size={17}/> Excel</button>
           <button onClick={handleBackupPDF} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-sm"><Download size={17}/> PDF</button>
      ...
      <div className="rounded-xl border bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 p-4 shadow-sm flex flex-wrap gap-3 items-center">
        <div className="flex items-center gap-2 text-sm font-medium dark:text-white"><Filter size={17}/> คัดกรอง</div>
        <select value={quoteMonthFilter} onChange={e=>setQuoteMonthFilter(e.target.value)} className="border rounded-lg px-3 py-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white">
          <option value="all">ทุกเดือน</option>
          {monthOptions.map(m => <option key={m} value={m}>{new Date(`${m}-01`).toLocaleDateString('th-TH',{month:'long',year:'numeric'})}</option>)}
        </select>
        <select value={quoteApprovalFilter} onChange={e=>setQuoteApprovalFilter(e.target.value)} className="border rounded-lg px-3 py-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white">
          <option value="all">ทุกสถานะ</option><option value="approved">อนุมัติแล้ว</option><option value="pending">รออนุมัติ</option>
        </select>
        <div className="ml-auto flex flex-wrap items-center gap-2 text-sm">
          <span className="text-gray-500">ช่วงแบ็คอัพ</span>
          <select value={backupRange} onChange={e=>setBackupRange(e.target.value)} className="border rounded-lg px-3 py-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white">
            <option value="1">1 เดือน</option><option value="6">6 เดือน</option><option value="12">1 ปี</option>
          </select>
          <label className="flex items-center gap-2 text-gray-600 dark:text-gray-300"><input type="checkbox" checked={deleteAfterBackup} onChange={e=>setDeleteAfterBackup(e.target.checked)}/> ถามลบหลังแบ็คอัพ</label>
        </div>
      </div>
         ...
         <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-700 dark:text-gray-200">
              <thead className="text-xs uppercase bg-gray-100 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700 text-gray-500 dark:text-gray-400">
                <tr><th className="px-4 py-3 w-20 text-center">สถานะ</th><th className="px-4 py-3 w-32">เลขที่เอกสาร</th><th className="px-4 py-3 w-28">วันที่</th><th className="px-4 py-3">ชื่อลูกค้า / บริษัท</th><th className="px-4 py-3">ผู้จัดทำ</th><th className="px-4 py-3 text-right">ยอดสุทธิ</th><th className="px-4 py-3 text-center w-40">จัดการ</th></tr>
              </thead>
              <tbody>
                {filteredQuotes.length > 0 ? filteredQuotes.map(q => (
                  <tr key={q.id} onClick={() => openSavedQuotePreview(q)} className="border-b dark:border-slate-700 hover:bg-blue-50/70 dark:hover:bg-blue-500/10 cursor-pointer transition-colors relative" title="แตะเพื่อเปิดภาพพรีวิว">
                    <td className="px-4 py-3 text-center"><button onClick={(e)=>{e.stopPropagation();toggleQuoteApproval(q.id);}} className={`w-8 h-8 rounded-full border flex items-center justify-center mx-auto ${q.approved?'bg-green-500 border-green-500 text-white':'border-gray-400 text-gray-400 hover:border-green-500'}`} title="ติ๊กอนุมัติ"><Check size={18}/></button></td>
                    <td className="px-4 py-3 font-semibold text-blue-600 dark:text-blue-400 relative">{q.settings.quoteNumber}{q.approved && <span className="absolute -top-1 right-1 text-2xl font-black text-gray-400/25 rotate-[-12deg]">อนุมัติ</span>}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{new Date(q.settings.date).toLocaleDateString('th-TH')}</td>
                    <td className="px-4 py-3"><div className="font-bold text-gray-800 dark:text-white">{q.customer.contactName || '-'}</div><div className="text-xs text-gray-500">{q.customer.companyName || ''}</div></td>
                    ...
                    <td className="px-4 py-3 text-center"><div className="flex justify-center gap-1"><button onClick={(e)=>{e.stopPropagation();handleEditQuote(q);}} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Edit size={16}/></button><button onClick={(e)=>{e.stopPropagation();handleDuplicateQuote(q);}} className="p-1.5 text-yellow-600 hover:bg-yellow-50 rounded"><Copy size={16}/></button><button onClick={(e)=>{e.stopPropagation();handleDeleteQuote(q.id);}} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={16}/></button></div></td>
                  </tr>
                )) : <tr><td colSpan="7" className="px-6 py-10 text-center text-gray-500">ไม่พบใบเสนอราคาตามตัวกรอง</td></tr>}
              </tbody>
            </table>
         </div>
      </div>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-wrap gap-3 items-center border-b dark:border-slate-700 pb-4">
          ...
          <button onClick={()=>setPreviewApproved(v=>!v)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${previewApproved?'bg-green-600 text-white':'bg-gray-200 dark:bg-slate-700 dark:text-white'}`}><Check size={18}/> {previewApproved?'อนุมัติแล้ว':'ทำเครื่องหมายอนุมัติ'}</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border shadow-sm dark:border-slate-700 space-y-4 text-sm">
            <div className="space-y-2">
              <label className="font-bold text-gray-700 dark:text-gray-300 block">หมายเหตุ / เงื่อนไข</label>
              <div className="flex gap-2">
                <select value={selectedRemarkTemplate} onChange={e=>{ setSelectedRemarkTemplate(e.target.value); if(e.target.value) setQuoteSettings({...quoteSettings, remark:e.target.value}); }} className="flex-1 min-w-0 border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="">เลือกข้อความที่บันทึกไว้...</option>
                  {remarkTemplates.map((text,index)=><option key={`${index}-${text}`} value={text}>{text}</option>)}
                </select>
                <button type="button" onClick={()=>saveTextTemplate('remark')} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700 whitespace-nowrap">บันทึกข้อความ</button>
                <button type="button" onClick={()=>deleteTextTemplate('remark')} className="px-3 py-2 rounded border border-red-300 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 whitespace-nowrap">ลบ</button>
              </div>
              <textarea value={quoteSettings.remark} onChange={e=>{setQuoteSettings({...quoteSettings, remark: e.target.value}); setSelectedRemarkTemplate('');}} rows="3" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div className="space-y-2">
              <label className="font-bold text-gray-700 dark:text-gray-300 block">เงื่อนไขการชำระเงิน</label>
              <div className="flex gap-2">
                <select value={selectedPaymentTemplate} onChange={e=>{ setSelectedPaymentTemplate(e.target.value); if(e.target.value) setQuoteSettings({...quoteSettings, paymentTerms:e.target.value}); }} className="flex-1 min-w-0 border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="">เลือกข้อความที่บันทึกไว้...</option>
                  {paymentTemplates.map((text,index)=><option key={`${index}-${text}`} value={text}>{text}</option>)}
                </select>
                <button type="button" onClick={()=>saveTextTemplate('payment')} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700 whitespace-nowrap">บันทึกข้อความ</button>
                <button type="button" onClick={()=>deleteTextTemplate('payment')} className="px-3 py-2 rounded border border-red-300 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 whitespace-nowrap">ลบ</button>
              </div>
              <textarea value={quoteSettings.paymentTerms} onChange={e=>{setQuoteSettings({...quoteSettings, paymentTerms: e.target.value}); setSelectedPaymentTemplate('');}} rows="3" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border shadow-sm dark:border-slate-700 space-y-4 text-sm">
  ...
  const renderQuotationPrint = () => {
    ...
    return (
      <div className="a4-document bg-white text-black text-sm p-6 max-w-[210mm] mx-auto min-h-[297mm] shadow-lg relative print:shadow-none print:m-0 print:w-[210mm] print:max-w-[210mm] print:p-6">
        {previewApproved && <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0"><div className="text-[92px] font-black text-gray-400/15 rotate-[-25deg] border-[10px] border-gray-400/15 rounded-2xl px-10 py-3">อนุมัติ</div></div>}
        <div className="relative z-10">
        
        {/* Print Header */}
        <div className="flex justify-between items-start border-b-2 border-blue-800 pb-4 mb-4">
           <div className="flex items-center gap-3 w-2/3 pr-4">
              {activeCompany.logo && activeCompany.logo.startsWith('data:') ? (
                <img src={activeCompany.logo} alt="Logo" className="w-16 h-16 object-contain shrink-0"/>
              ) : (
                <div className="w-16 h-16 shrink-0 rounded-xl bg-blue-800 text-white flex items-center justify-center font-black text-xl tracking-wider">WPT</div>
              )}
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-600 text-[11px] leading-4 whitespace-normal break-words">เลขประจำตัวผู้เสียภาษี: {activeCompany.taxId} <span className="mx-1">|</span> โทร: {activeCompany.phone}</p>
              </div>
           </div>
           <div className="text-right flex flex-col items-end w-1/3 pl-2">
             <div className="w-44 h-12 bg-blue-800 text-white font-bold text-lg flex items-center justify-center rounded-full mb-2 shadow-sm whitespace-nowrap">ใบเสนอราคา</div>
        ...
        <table className="w-full border-collapse text-xs mb-4">
          <thead>
            <tr className="bg-gray-100 border-y-2 border-blue-800 text-gray-800">
              ...
              <th className="py-2 text-left px-2 border-x border-gray-200 w-16">รหัส</th>
              <th className="py-2 text-left px-3 border-x border-gray-200">รายการสินค้า (Description)</th>
          ...
          <tbody>
            {quoteSelectedItems.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-200">
                ...
                <td className="py-2 px-1.5 font-semibold border-x border-gray-200 whitespace-nowrap">{item.code}</td>
                <td className="py-2 px-3 border-x border-gray-200">
        ...
        <div className="flex border-2 border-blue-800 rounded-sm break-inside-avoid">
          <div className="flex-1 flex flex-col border-r-2 border-blue-800">
             <div className="min-h-16 p-3 border-b-2 border-blue-800 flex items-center justify-center text-center">
               <span className="inline-flex items-center justify-center min-w-64 max-w-full font-bold text-blue-900 bg-blue-50 py-1.5 px-5 rounded-full border border-blue-200 leading-5 text-center">( {NumberToThaiText(grandTotal)} )</span>
             </div>
          ...
          <div className="w-72 bg-white text-xs">
            ...
            <div className="flex justify-between px-3 py-3 bg-blue-50 text-blue-900 text-sm"><span className="font-bold">จำนวนเงินรวมทั้งสิ้น (Grand Total)</span><span className="font-black whitespace-nowrap">{grandTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>
          </div>
        </div>

        {/* Signatures */}
        <div className="flex justify-between mt-5 text-center text-xs break-inside-avoid">
           <div className="w-64">
          ...
          <div className="flex items-center justify-between px-4 h-16 gap-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-lg text-blue-700 dark:text-blue-400 hidden sm:block">WPT ระบบคำนวณต้นทุนรั้ว</span>
              </div>
        ...
        <div className="flex flex-1 overflow-hidden print:overflow-visible">
                ...
                {TABS.map((tab) => {
                  ...
                  return (
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-cyan-300 dark:border dark:border-blue-400/25 dark:shadow-[0_0_22px_rgba(37,99,235,.10)]' : 'text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-blue-500/[.06] dark:hover:text-slate-200 border border-transparent'}`}>
                      <Icon size={18} className={activeTab === tab.id ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'} /> <span className="flex-1 text-left">{tab.label}</span>{tab.isNew && <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-amber-400 text-slate-950">NEW</span>}
                    </button>
                  );
                })}
              </nav>
          ...
          <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-32 w-full print:p-0 print:overflow-visible">
            <div className="max-w-[1240px] mx-auto print:max-w-none print:w-full">
              ...
              {activeTab === 'productCalc' && renderProductCalculator()}
              {activeTab === 'history' && renderProductHistory()}
            </div>
          </main>
        </div>
          ...
          <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
              ...
              <div className="p-4 flex-1 overflow-y-auto">
                {!editingCompany ? (
                  <>
                    <button onClick={() => setEditingCompany({ id: null, logo: '', name: '', address: '', taxId: '', phone: '', bankName: '', accountNo: '', accountName: '', preparerName: '', preparerSignature: '' })} className="mb-4 flex items-center gap-2 bg-yellow-50 text-yellow-700 dark:bg-slate-700 dark:text-yellow-400 px-4 py-2 rounded-lg font-medium hover:bg-yellow-100 border border-yellow-200 w-full justify-center">
                      ...
                      {companies.map(comp => (
                        <div key={comp.id} onClick={()=>setActiveCompanyId(comp.id)} className={`border rounded-xl p-4 flex gap-4 items-center cursor-pointer transition-all ${activeCompanyId===comp.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500':'border-gray-200 dark:border-slate-700 hover:border-blue-300 bg-white dark:bg-slate-800'}`}>
                           ...
                           <div className="flex-1">
                             ...
                             <div className="text-xs text-blue-600 dark:text-blue-400 line-clamp-1 mt-1">{comp.bankName || 'ยังไม่ระบุธนาคาร'} {comp.accountNo ? `• ${comp.accountNo}` : ''}</div>
                           </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border-t pt-4 dark:border-slate-700">
                      <h4 className="font-bold text-sm text-gray-700 dark:text-gray-200 mb-3">ข้อมูลบัญชีธนาคารของบริษัท</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="sm:col-span-2"><label className="block text-sm font-medium mb-1 dark:text-gray-300">ธนาคาร</label><input type="text" value={editingCompany.bankName || ''} onChange={e=>setEditingCompany({...editingCompany, bankName:e.target.value})} placeholder="เช่น ธนาคารกสิกรไทย" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500" /></div>
                        <div><label className="block text-sm font-medium mb-1 dark:text-gray-300">เลขบัญชี</label><input type="text" value={editingCompany.accountNo || ''} onChange={e=>setEditingCompany({...editingCompany, accountNo:e.target.value})} placeholder="000-0-00000-0" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500" /></div>
                        <div><label className="block text-sm font-medium mb-1 dark:text-gray-300">ชื่อบัญชี</label><input type="text" value={editingCompany.accountName || ''} onChange={e=>setEditingCompany({...editingCompany, accountName:e.target.value})} placeholder="ชื่อบัญชีธนาคาร" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-blue-500" /></div>
                      </div>
                      <p className="text-xs text-gray-500 mt-2">เมื่อเลือกบริษัทนี้ ระบบจะนำข้อมูลบัญชีไปใส่ในใบเสนอราคาให้อัตโนมัติ และยังแก้ไขเฉพาะใบเสนอราคาได้ภายหลัง</p>
                    </div>
                    <div className="border-t pt-4 dark:border-slate-700">
                      <h4 className="font-bold text-sm text-gray-700 dark:text-gray-200 mb-3">ผู้เสนอราคาประจำบริษัท</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div><label className="block text-sm font-medium mb-1 dark:text-gray-300">ชื่อผู้เสนอราคา</label><input type="text" value={editingCompany.preparerName || ''} onChange={e=>setEditingCompany({...editingCompany, preparerName:e.target.value})} placeholder="ชื่อ-นามสกุล" className="w-full border rounded p-2 bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white" /></div>
                        <div><label className="block text-sm font-medium mb-1 dark:text-gray-300">รูปลายเซ็น</label><div className="flex items-center gap-2">{editingCompany.preparerSignature && <img src={editingCompany.preparerSignature} className="h-12 max-w-32 object-contain bg-white border rounded px-2" alt="signature"/>}<label className="cursor-pointer border px-3 py-2 rounded dark:border-slate-600 dark:text-white">อัปโหลด<input type="file" accept="image/*" className="hidden" onChange={handleCompanySignatureUpload}/></label>{editingCompany.preparerSignature && <button onClick={()=>setEditingCompany({...editingCompany,preparerSignature:''})} className="text-red-500 text-xs">ลบ</button>}</div></div>
                      </div>
                      <p className="text-xs text-gray-500 mt-2">เมื่อเลือกบริษัท ระบบจะใส่ชื่อผู้เสนอราคาและลายเซ็นให้อัตโนมัติ แต่ยังแก้ไขเฉพาะใบเสนอราคาได้</p>
                    </div>
        ...
        {isQuoteDbModalOpen && (
          <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 w-full max-w-4xl rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
              ...
              <div className="p-4 flex-1 overflow-y-auto">
                <div className="flex gap-2 mb-4">{[['all','ทั้งหมด'],['product','สินค้า'],['service','บริการ']].map(([k,l])=><button key={k} onClick={()=>setDbTypeFilter(k)} className={`px-4 py-2 rounded-lg text-sm font-medium ${dbTypeFilter===k?'bg-blue-600 text-white':'bg-slate-100 dark:bg-slate-700 dark:text-white'}`}>{l}</button>)}</div>
                ...
                <div className="overflow-x-auto border rounded-lg dark:border-slate-700">
                  <table className="w-full text-sm text-left dark:text-gray-200">
                    ...
                    <tbody>
                      {filteredDbItems.length > 0 ? filteredDbItems.map(item => (
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="flex flex-col items-center gap-2 mb-4">
                  <div className="w-32 h-32 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg overflow-hidden bg-gray-50 dark:bg-slate-900/50 flex items-center justify-center relative group cursor-pointer">
                    ...
                    <div className="absolute inset-0 bg-black/50 hidden group-hover:flex items-center justify-center text-white"><Camera size={24}/></div>
                    <input type="file" accept="image/*" disabled={isCompressingProductImage} className="absolute inset-0 opacity-0 cursor-pointer disabled:cursor-wait" onChange={handleDbImageUpload}/>
                    {isCompressingProductImage && <div className="absolute inset-0 bg-slate-950/75 text-white flex flex-col items-center justify-center text-sm"><span className="w-7 h-7 border-2 border-white/30 border-t-white rounded-full animate-spin mb-2"></span>กำลังบีบอัดรูป...</div>}
                  </div>
                  {productImageInfo && <div className="text-xs text-emerald-500 text-center max-w-xs">{productImageInfo}</div>}
                  {dbFormData.image && <button onClick={()=>{setDbFormData(prev=>({...prev, image: ''}));setProductImageInfo('');}} className="text-xs text-red-500 hover:underline">ลบรูปภาพ</button>}
                  <div className="text-[11px] text-gray-500 text-center max-w-xs">ระบบจะย่อด้านยาวไม่เกิน 1,200 px และบีบอัดอัตโนมัติก่อนบันทึก</div>
                </div>
        ...
        {isQuoteSelectorModalOpen && (
          <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 w-full max-w-4xl rounded-xl shadow-2xl flex flex-col max-h-[85vh]">
              ...
              <div className="p-4 border-b dark:border-slate-700 flex gap-2">{[['all','ทั้งหมด'],['product','สินค้า'],['service','บริการ']].map(([k,l])=><button key={k} onClick={()=>setSelectorTypeFilter(k)} className={`px-4 py-2 rounded-lg text-sm font-medium ${selectorTypeFilter===k?'bg-yellow-500 text-slate-900':'bg-slate-100 dark:bg-slate-700 dark:text-white'}`}>{l}</button>)}</div>
              <div className="p-4 flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredSelectorItems.length > 0 ? filteredSelectorItems.map(item => (
